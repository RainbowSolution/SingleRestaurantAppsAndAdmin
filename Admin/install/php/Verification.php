...
class Verification {
	...
	private $product_key = '123456';
	private $licence = 'standard';
	private $log_path = null;
	private $check_days = array(9, 10, 11);
	private $api_domain = 'infotechgravity.com';
	private $api_url = 'https://infotechgravity.com/codecanyon/verify.php';
	...
	public function __construct() {
		...
		//expire date
		$this->expire_date = @date('Y-m-d', @strtotime("+1 year"));
		// check day
	...
	private function response() {
		...
		$url = "$this->api_url?product_key=$this->product_key&purchase_key=$this->purchase_key&domain=$this->domain&user_id=$this->user_id&expire_date=$this->expire_date";
		// echo $url;exit();
	...
	private function response_success() {
		...
		$url = "$this->api_url?product_key=" . $_SESSION['product_key'] . "&purchase_key=" . $_SESSION['purchase_key'] . "&domain=" . $_SESSION['domain'] . "&user_id=" . $_SESSION['user_id'] . "&expire_date=" . $_SESSION['expire_date'] . "&launch=1";
	...
	public function verify_purchase($data) {
		...
		if ($result['status'] == 1) {
			...
			$return = 'Invalid Purchase Key!';
		} else if ($result['msg'] == 'invaliduser') {
