...
class FileWrite {
	...
	function databaseConfig($path = null, $data = []) {
		if (file_exists($path['template_path'])) {
			...
			$new = str_replace("{PURCHACE_KEY}", $_SESSION['purchase_key'], $new);
			$new = str_replace("{DOMAIN}", $_SESSION['domain'], $new);
			$new = str_replace("{USERID}", $_SESSION['user_id'], $new);
			$new = str_replace("{EXPIREDATE}", @$_SESSION['expire_date'], $new);
